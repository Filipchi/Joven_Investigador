...
def cargarposicion():            
    ...
    for i in range(nparticulas):
        ...
        x[i] += aP*np.cos(x[i]) # Perturbacion
...
def cargarvelocidad(tdistribucion,vh):       
    ...
    if(tdistribucion == 0):  
        ...
        for i in range(nparticulas):    
            ...
            f = 0.5 * (np.exp(-(vtemporal - vh)*(vtemporal - vh) / 2.0) + np.exp(-(vtemporal + vh)*(vtemporal + vh) / 2.0))
         ...
         vx = vv*np.cos(2*np.pi*x/longitud_malla)
         vy = vv*np.sin(2*np.pi*x/longitud_malla)
    ...
    v += vP*np.sin(2*np.pi*x/longitud_malla)
...
def densidad(qe):      
   ...
   j1 = np.dtype(np.int32) # Asegura que la variable permanezca entera
   j2 = np.dtype(np.int32)    
   
   # Factor de ponderacion de carga 
   re = qe/dx 
   # Densidad electronica 
   rhoe = np.zeros(npuntos_malla+1)   
...
def campo():          
  ...
  xintegrar = dx * np.arange(npuntos_malla+1)
  Ex = sp.integrate.cumulative_trapezoid(rhot,xintegrar, initial = xintegrar[0])
  edc = np.sum(Ex)
...
def movimientoparticulas():  
    ...
    if (Bo != 0):
          v_menos = np.zeros((nparticulas,3))
          v_prima = np.zeros((nparticulas,3))
          v_mas = np.zeros((nparticulas,3))
          vz = np.zeros(nparticulas)
          B = np.array([0,0,Bo])
...
def diagnosticos():
    ...
    xgrafica = dx * np.arange(npuntos_malla+1)
...
#                            
            if (ifase > 0):
              if (np.fmod(itiempo,ifase) == 0):  
                # Se grafica el espacio de fase en el paso dado por el contador ifase:
                plt.figure(3)
                if (itiempo >0 ): plt.cla()
                v1 = np.zeros(nparticulas)
                v2 = np.zeros(nparticulas)
                x1 = np.zeros(nparticulas)
                x2 = np.zeros(nparticulas)
                for i in range(nparticulas):
                    if (v[i-1]>v[i]):
                        v1[i]=v[i]
                        x1[i]=x[i]
...
##                    np.savetxt(fn_vdist, np.column_stack((v,F)),fmt=('%1.4e','%1.4e'))   
...
#  plt.ylim(0,25)
  ...
  True:
    ...
    np.savetxt('energies.out', np.column_stack((t,EnergiaP,EnergiaK,EnergiaT)),fmt=('%1.4e','%1.4e','%1.4e','%1.4e'))   
...
def relacion_dispersion(distribucion):
    ...
    omegapp = omegap*np.ones(nparticulas)
    
    # Se calcula la frecuencia angular y espacial minima y maxima (Ver codigo KEMPO1):
    omega_min = 2*np.pi/(dt)/2/(npasos_temporales/2)
    omega_max = omega_min*(npasos_temporales/2)
    k_min = 2*np.pi/(npuntos_malla)
    k_max = k_min*((npuntos_malla/2)-1)
    
    # Se crean los vectores de frecuencias espacial y angular teoricas y simuladas:
    k_t =np.linspace(0,k_max,nparticulas)
    k_simulada = np.linspace(-k_max,k_max,nextpow2(npuntos_malla))
    omega_t = np.linspace(0,longitud_malla,npuntos_malla+1)
    omega_simulada = np.linspace(-omega_max,omega_max,nextpow2(npuntos_malla))
    
    # Se genera una matriz de frecuencias angular y espacial:
    K, W = np.meshgrid(k_simulada,omega_simulada)    
    ...
    if(distribucion==1):
       ...
       omega_t = -1j*(np.sqrt((vh*k_t)**2 + omegap**2 - omegap*np.sqrt(4*(k_t*vh)**2 + omegap**2)))
...
x = np.zeros(nparticulas)  
y = np.zeros(nparticulas)
z = np.zeros(nparticulas)
x_aux = np.empty((npasos_temporales+1,nparticulas))
y_aux = np.empty((npasos_temporales+1,nparticulas))
v = np.zeros(nparticulas)     
vx = np.zeros(nparticulas)
vy = np.zeros(nparticulas)

# Arreglo de mallas
rhoe = np.zeros(npuntos_malla+1)        # Densidad electronica 
rhoi = np.zeros(npuntos_malla+1)        # Densidad ionica 
rhot = np.zeros(npuntos_malla+1)        # Densidad total
Ex   = np.zeros(npuntos_malla+1)        # Campo electrico  

# Historial de energia
EnergiaK   = np.zeros(npasos_temporales+1)
EnergiaP   = np.zeros(npasos_temporales+1)
EnergiaT   = np.zeros(npasos_temporales+1)

# Matriz espacio-temporal del campo E:
E_acumulador = np.empty((npuntos_malla+1,npasos_temporales+1))
# Numero de onda y frecuencia relacion de dispersion
k_s = np.linspace(0,longitud_malla,npuntos_malla+1)          # Simulada
omega_t = np.linspace(0,longitud_malla,npuntos_malla+1)      # Teorica
omega_s = np.linspace(0,longitud_malla,npuntos_malla+1)      # Simulada
...
igrafica = int(np.pi/dt/16) # Frecuencia de las capturas de pantalla de las graficas
ifase = igrafica
ivdist = igrafica
